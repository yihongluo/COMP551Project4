...
def participant_manager():
    ...
    for i in range(20):
        ...
        participant = threading.Thread(name='participant' + str(i), target=ffn, args = (training_data_participant, test_data, server))
        participant.start()
        start = end
        end = end + 1000
        #break
...
def ffn(training_data, test_data, server):
    ...
    net = network.Network([784, 30, 10], 1, 1, server)
    #net = network.Network([784, 30, 10], 0.1, 0.1, server)
    net.SGD(training_data, 50, 1, 2, test_data=test_data)
