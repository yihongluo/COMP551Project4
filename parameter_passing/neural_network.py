...
class NeuralNetwork(object):
    ...
    def __init__(self, dimension, collaborate, existing_weights, existing_biases):
        self.totalLayers = len(dimension)
        self.dimension = dimension
        
        if collaborate:
            self.biases = existing_biases
            self.weights = existing_weights
        else:    
            self.biases = [np.random.randn(y, 1) for y in dimension[1:]]
            self.weights = [np.random.randn(y, x)
                        for x, y in zip(dimension[:-1], dimension[1:])]
